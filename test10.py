    ...
    try:
        ...
        response = vonage_client.voice.create_call({
            ...
            'ncco': [
                {
                    'action': 'record',
                    'eventUrl': [f'https://e1d1-138-199-40-175.ngrok-free.app/vonage_recording?call_id={call_id}'],
                    'format': 'mp3'
                },
                {
                    'action': 'connect',
                    'endpoint': [
                        {
                            'type': 'websocket',
                            'uri': f'wss://e1d1-138-199-40-175.ngrok-free.app/ws?call_id={call_id}',
            ...
            'event_url': [f'https://e1d1-138-199-40-175.ngrok-free.app/vonage_call_status?call_id={call_id}'],
            'event_method': 'POST'
        })
...
async def generate_and_send_speech(websocket: WebSocket, conversation_history: list, human_response: str, agent_response: str):
    try:
        ...
        while not all(key in results for key in ["conversation_summary", "key_points", "specific_context", "current_goal_review"]):
        ...
        sales_utterance_response = await sales_api.do(
            ...
            specific_context=results["specific_context"],
            current_goal_review=results["current_goal_review"],
            transition_statement_status=sales_api.sales_agent.transition_statement_status,
        )
